...
export async function readEmails(credentials: any, parameters: any) {
...
export async function receiveTelegramMessages(credential: any, parameters: any) {
  try {
    ...
    const allMessages = (response.data.result || [])
      .filter((update: any) => update.message && update.message.chat && update.message.chat.id == CHAT_ID);
console.log('All messages for chat:', allMessages);
    const lastMessages = allMessages.slice(-limit).map((update: any) => ({
